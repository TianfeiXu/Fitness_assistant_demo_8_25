...
JSON_BLOCK = re.compile(r"\{[\s\S]*\}")  # 放在文件顶部或函数外也行, 从回复里保险抓 JSON 块
    ...
    for attempt in range(retries + 1):
        try:
            resp = client.chat.completions.create(
                ...
                messages=[...],
                temperature=0.0,
                response_format={"type": "json_object"},  # 你之前用到就保留
            )
...
def update_heat_with_items(heat_df: pd.DataFrame, items: List[Dict[str, Any]]) -> pd.DataFrame:
    """
    将热度从 heat_df 的最后日期按半衰期推进到 items 中的最晚日期（或今天），
    再把 items 当天各部位的增量叠加到目标日期上，并保存到 CSV。
    """
    # —— 1) 目标日期：items 里最晚的 date（如果 items 为空则用今天） ——
    target_date = max([dt.date.fromisoformat(i["date"]) for i in items], default=dt.date.today())

    # —— 2) 初始化：空表时从“昨天”或今天建一行全 0（保持和你的 ensure_dirs 逻辑一致） ——
    if heat_df is None or heat_df.empty:
        heat_df = pd.DataFrame([{"date": dt.date.today(), **{bp: 0.0 for bp in BODY_PARTS}}])

    # —— 3) 确保 date 列是 date 类型；并按日期排序后确定“当前行” ——
    heat_df = heat_df.copy()
    if not pd.api.types.is_datetime64_any_dtype(heat_df["date"]):
        heat_df["date"] = pd.to_datetime(heat_df["date"]).dt.date

    # 用最大日期更稳，不依赖“最后一行就是最新”
    last_date = heat_df["date"].max()
    # 取 last_date 对应的那一行作为当前状态
    current_row = heat_df.loc[heat_df["date"] == last_date].iloc[0].to_dict()
    # 保证所有部位都是 float
    for bp in BODY_PARTS:
        current_row[bp] = float(current_row.get(bp, 0.0))

    # —— 4) 按日推进：把 current_row 衰减到 target_date，每推进一天就 append 一行 ——
    while last_date < target_date:
        last_date = last_date + relativedelta(days=1)
        decayed = {bp: current_row[bp] * HEAT_DECAY for bp in BODY_PARTS}
        row = {"date": last_date, **decayed}
        heat_df = pd.concat([heat_df, pd.DataFrame([row])], ignore_index=True)
        current_row = row  # 滚动窗口

    # —— 5) 叠加增量：把 items 当天的各部位“训练量 proxy”加到 target_date 那行 ——
    increments = {bp: 0.0 for bp in BODY_PARTS}
    for it in items:
        bp = (it.get("body_part") or "other").lower()
        if bp not in BODY_PARTS:
            bp = "other"
        increments[bp] += compute_volume(it)  # 你的 compute_volume 已处理 cardio = 0

    # 找到 target_date 对应的索引并叠加
    idx_list = heat_df.index[heat_df["date"] == target_date].tolist()
    if not idx_list:
        # 理论上不会发生；兜底建一行
        new_row = {"date": target_date, **{bp: 0.0 for bp in BODY_PARTS}}
        heat_df = pd.concat([heat_df, pd.DataFrame([new_row])], ignore_index=True)
        idx_list = [heat_df.index[-1]]
...
if not heat_df.empty:
    ...
    heat_long = pd.DataFrame({"body_part": list(heat_today.keys()),
                              "heat": list(heat_today.values())})
    ...
    bar = alt.Chart(heat_long).mark_bar().encode(
        ...
        y=alt.Y(
            "heat:Q",
            title="Heat (proxy)",
            scale=alt.Scale(domain=[0, max(HEAT_HIGH * 1.2, heat_long["heat"].max() if not heat_long.empty else 3.5)])
        ),
